...
- `OAuth` -> `Live`: `.env` に `TRAQ_CLIENT_ID` を設定すると traQ OAuth でログインし、`GET /api/v3/ws` の `MESSAGE_CREATED` と `USER_VIEWSTATE_CHANGED` を `trigger` イベントに変換します。
  ...
  - 全体の閲覧状況は `GET /api/v3/channels/{channelId}/viewers` を一部チャンネルに対して定期取得し、`viewers` SSE と backend ログに出します。メッセージが多いチャンネルほど選ばれやすくします。
...
- 閲覧状態ストリームの実装メモ: [`docs/agents/viewstate-stream.md`](docs/agents/viewstate-stream.md)
